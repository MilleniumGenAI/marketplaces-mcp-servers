...
class YmProvider(BaseMarketplaceProvider):
    def __init__(self):
        self.provider_id = "apify"
        self.actor_id = os.environ.get("YM_APIFY_ACTOR_ID", "zen-studio~yandex-market-scraper-parser").strip().replace("/", "~")
        
        self.proxy_use = os.environ.get("YM_APIFY_PROXY_USE", "1").strip().lower() in {"1", "true", "yes"}
        self.proxy_group = os.environ.get("YM_APIFY_PROXY_GROUP", "").strip()
        self.proxy_country = os.environ.get("YM_APIFY_PROXY_COUNTRY", "").strip().upper()
        
        self.client = ApifyClient(
            token=os.environ.get("APIFY_TOKEN", "").strip(),
            base_url=os.environ.get("YM_APIFY_BASE_URL", "https://api.apify.com/v2"),
            timeout_sec=float(os.environ.get("YM_APIFY_TIMEOUT_SEC", "120")),
            retries=max(1, int(os.environ.get("YM_APIFY_RETRIES", "3"))),
        )
    ...
    def _safe_link(self, v: Any) -> str:
        ...
        if text.startswith("//"):
            return f"https:{text}"
        if text.startswith("/"):
            return urljoin("https://market.yandex.ru", text)
        return text
    ...
    def _normalize_row(self, row: Dict[str, Any]) -> Optional[ProductItem]:
        ...
        link = self._safe_link(self._pick_first(raws, ("link", "url", "productUrl", "offerUrl", "marketUrl")))
        ...
        brand = str(self._pick_first(raws, ("brand", "vendor", "manufacturer")) or "").strip()
